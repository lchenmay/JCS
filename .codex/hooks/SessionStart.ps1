...
try {
    ...
    $context = "$name project policy active: mode=$mode, risk=$risk, changed-files=$($changedFiles.Count), write-scope=repository-only, outside-repository=ignore. Dependency repositories are read-only. Runtime permission mode '$permissionMode' is not managed by this repository. Verification entry point: $verifyCommand."
